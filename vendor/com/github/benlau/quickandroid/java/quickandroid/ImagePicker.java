...
public class ImagePicker {
    ...
    static void takePhoto(Map message) {
        ...
        File img = new File(storageDir.getAbsolutePath() + "/" + timeStamp + ".jpg");
        mPhotoUri = Uri.fromFile(img);
    ...
    static private void onActivityResult(Map message) {
        if (((Integer) message.get("resultCode")) != Activity.RESULT_OK) {
            return;
        }
    ...
    static private void importImageFromFileUri(Uri fileUri) {
    ...
    static private void importImageFromContentUri(Uri uri) {
        ...
        for (int i = 0 ; i < cursor.getCount(); i++) {
            ...
            if (path == null) {
                Log.d(TAG, "importImageFromContentUri: The path of is null. The image not on storage.");
                continue;
            }
